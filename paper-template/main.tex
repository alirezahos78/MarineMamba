...
\begin{itemize}
  ...
  \item experiments on AQUA20, Sea Animals 23, and Fish4Knowledge showing strong top-1 accuracy with substantially fewer trainable parameters than fVim-tiny, which is the Vim-tiny model we fine-tuned on the marine datasets under the same training settings as MarineMamba;
...
The class-frequency distributions exhibit a pronounced long right-tailed pattern, indicating substantial class imbalance across the evaluated datasets. This imbalance is especially severe in Fish4Knowledge, where a small number of classes account for a disproportionately large fraction of samples while many classes remain sparsely represented. AQUA20 also shows a clear right-tailed distribution, although less extreme than Fish4Knowledge. In contrast, Sea Animals is comparatively more balanced, with class counts varying within a narrower range.

\begin{figure}[t]\label{fig:dataset}
  \centering
  \caption{Class-wise train/test sample distribution showing long right-tailed imbalance, most prominently in Fish4Knowledge and AQUA20.}
...
\begin{table}[t]
\centering
\caption{Selected results on AQUA20 (20 classes). Rows marked $\dagger$ are fully fine-tuned baselines reported by Fuad et al.~\cite{fuad2026aqua20}; $\ddagger$ denotes backbones fine-tuned by Daga et al.~\cite{daga2026aquaselect}. Parameter counts for frozen-CLIP rows and MarineMamba are trainable parameters. fVim-tiny and MarineMamba are our own models, trained by us under identical conditions.}
\label{tab:aqua20}
\begin{tabular}{llccc}
\toprule
...
\begin{table}[t]
\centering
\caption{Results on Fish4Knowledge (23 classes). The top block is our own controlled comparison on the raw 881:1-imbalanced 80/20 split, with both rows trained by us under an identical protocol on an identical split. The bottom block lists published results reported by Prasetyo et al.$^{\S}$ .}
\label{tab:fish4k}
\begin{tabular}{lccc}
\toprule
Method & Params & Top-1 Acc. & Macro F1 \\
...
\vspace{2pt}
\footnotesize
While we use the pretrained weights released with Vim~\cite{zhu2024vim}, the fine-tuning strategy, learning-rate schedule, data-augmentation choices, and early-stopping criterion used for this reference are our own implementation work.
\end{table}
...
Macro-F1 qualifies the AQUA20 result. MarineMamba reaches $83.38\pm1.95\%$, below ConvNeXt's $88.92\%$ and only $0.42$ points above the frozen-CLIP MLP. AQUA20 contains several classes with only ten test images, so top-1 is dominated by majority-class behavior. On Sea Animals 23, where the class distribution is less skewed, the pattern is stronger: MarineMamba reaches $95.90\pm0.80\%$ top-1 and $95.24\pm0.94\%$ macro-F1, exceeding both the ConvNeXt-Tiny baseline and our own fVim-tiny reference by clear margins.

Fish4Knowledge is the hardest imbalance setting. MarineMamba reaches $98.25\pm0.26\%$ top-1 and $87.14\pm1.33\%$ macro-F1 under an 881:1 training imbalance, trailing our own fVim-tiny reference by $1.55$ top-1 points while training $6.8\times$ fewer parameters and keeping CLIP frozen. Published results from Prasetyo et al. are listed in \Cref{tab:fish4k} for context only: their curated, class-balanced split and different preprocessing make them not directly comparable to this raw split, and top-1 on a balanced split is not equivalent to top-1 under an 881:1 imbalance.
...
Spiral scanning gives a modest mean gain over raster order ($+0.53$ points) but a large stability gain, reducing the standard deviation from $0.82\%$ to $0.13\%$. The dual-branch design behaves similarly: the coarse-only branch nearly matches the full model's mean ($-0.10$ points) but with markedly higher variance ($0.51\%$ vs.\ $0.13\%$). Finally, focal loss improves this head by $0.61$ points over cross-entropy, but it is not a general cure for imbalance, as the same loss lowers the frozen-CLIP MLP in \Cref{tab:aqua20}.

\subsection{Embedding Visualization}
\label{sec:umap}

To qualitatively examine how the trainable head reshapes the frozen representation, we project six embedding spaces to two dimensions with UMAP~\cite{mcinnes2018umap} (\texttt{n\_neighbors}$=15$, \texttt{min\_dist}$=0.1$, and a $50$-component PCA pre-reduction), plotting train and test points together. The spaces are the two frozen CLIP spatial grids after mean pooling ($768$-d, ViT-B/16 and ViT-B/32), the fine and coarse MarineMamba branch descriptors ($128$-d each), their concatenation $\mathbf{h}=[\mathbf{h}_{16};\mathbf{h}_{32}]$ ($256$-d, the vector the classifier actually receives), and a $64\times64\times3$ raw-pixel baseline. \Cref{fig:umap-aqua20,fig:umap-sea23,fig:umap-fish4k} show the result for AQUA20, Sea Animals~23, and Fish4Knowledge.
...
\begin{figure}[t]
  ...
  \caption{UMAP projection of six embedding spaces on AQUA20 (train $\cdot$, test $\times$). Panels 1--2: frozen CLIP ViT-B/16 and ViT-B/32 grids after mean pooling. Panels 3--5: MarineMamba fine, coarse, and fused fine$\oplus$coarse descriptors. Panel 6: $64\times64\times3$ raw-pixel baseline. Species stay entangled in the CLIP and raw-pixel spaces but form compact, largely class-pure clusters in the fused MarineMamba space.}
...
\begin{figure}[t]
  ...
  \caption{UMAP projection of the same six embedding spaces on Sea Animals~23 (train $\cdot$, test $\times$). The fused MarineMamba space (panel 5) isolates most of the $23$ species into well-separated clusters --- the clearest separation among the three datasets, consistent with the strongest top-1 and macro-F1.}
...
\begin{figure}[t]
  ...
  \caption{UMAP projection of the same six embedding spaces on Fish4Knowledge (train $\cdot$, test $\times$). Under the 881:1 imbalance, the fused MarineMamba space (panel 5) separates the frequent species that dominate the split, while several rare classes remain small and adjacent, mirroring the high top-1 but lower macro-F1.}
...
This paper presented MarineMamba, a lightweight classifier that combines frozen CLIP ViT-B/16 and ViT-B/32 features with a spiral-scan Mamba head for underwater and aquatic species recognition. The method trains only 1.03M parameters, yet exceeds our own fully fine-tuned fVim-tiny reference on AQUA20 and Sea Animals 23 and remains competitive with it on the raw, highly imbalanced Fish4Knowledge split. The ablations clarify the mechanism: the main gain over a frozen-CLIP MLP comes from using the spatial grid, and the Mamba layer is most important for propagating the injected CLIP feature vector across spatial tokens before pooling.
