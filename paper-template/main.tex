...
\begin{abstract}
Recognizing marine and aquatic species from underwater imagery is a demanding fine-grained task: categories differ by subtle shape, fin, and texture cues, and images are degraded by wavelength-dependent attenuation, scattering, and non-uniform illumination. Fully fine-tuning large CNN, Transformer, or state-space backbones on such small, uneven collections is a fragile default. We instead keep the representation fixed and learn only how to read it. MarineMamba is a lightweight classifier built on two frozen CLIP encoders, ViT-B/16 and ViT-B/32, that supply complementary fine- and coarse-grained evidence. A compact dual-branch state-space head is the only trainable component: each branch injects the global CLIP feature vector into the spatial grid, scans the patch tokens in a locality-preserving bidirectional spiral order through a shared Mamba layer, and refines the result with channel-and-spatial attention before the two granularities are fused. Trained with only about a million parameters and without ever updating CLIP, MarineMamba is competitive with our own far heavier fully fine-tuned Vision-Mamba model: it exceeds that reference on two aquatic benchmarks and stays close to it on a raw fish-recognition split evaluated without rebalancing. Ablations attribute the gain to one mechanism in particular: the Mamba layer propagates the injected CLIP feature vector across spatial tokens before pooling, something a token-wise feed-forward alternative cannot achieve.
...
We propose MarineMamba, a dual-branch classifier for underwater and aquatic species recognition. The method extracts frozen CLIP ViT-B/16 and ViT-B/32 spatial grids and feature vectors, projects each granularity into a compact hidden space, prepends the corresponding CLIP feature vector to the spatial tokens, and processes the sequence with a bidirectional spiral-scan Mamba block. After restoring the spatial layout, channel-and-spatial attention refines each branch; the two descriptors are concatenated and classified. Because the CLIP encoders are never updated, MarineMamba trains only 1.03M parameters, which make the model very suitable for edge devices. We also explore fine-tuning Vim-tiny on the marine datasets, which yields good results but at the cost of a considerable number of trainable parameters.
...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.78\linewidth]{figures/mamba.drawio.jpg}
...
\begin{figure}[h]
  \centering
  \includegraphics[width=0.9\linewidth]{figures/umap_aqua20.jpg}
...
\begin{figure}[h]
  \centering
  \includegraphics[width=0.9\linewidth]{figures/umap_sea23.jpg}
...
\begin{figure}[h]
  \centering
  \includegraphics[width=0.9\linewidth]{figures/umap_fish4k.jpg}
